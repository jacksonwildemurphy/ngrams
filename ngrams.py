...
# adds a tokenized array to the unigram frequency table
...
# adds a tokenized array to the bigram frequency table, prepending "phi" to beginning of sentence
...
def prob_wi_given_w_iMinus1(w_i, w_iMinus1):
    bigram = w_i + " " + w_iMinus1
...
def print_sentence_prob_bigram_unsmoothed(sentence, bi_frequencies_sum):
    ...
    for i in range(1, len(token_arr)):
        ...
        prob = prob_wi_given_w_iMinus1(w_i, w_iMinus1)
        if prob == 0:
            print("Unsmoothed Bigrams, logprob(S) = undefined")
            return
        logprob = np.log2(prob_wi_given_w_iMinus1(w_i, w_iMinus1))
        logprob_sum += token_prob
